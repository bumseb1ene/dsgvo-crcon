...
from rcon.privacy import format_retention_results, purge_old_personal_data
...
@cli.command(name="privacy_purge")
@click.option("--execute", is_flag=True, help="Actually delete rows. Default is dry-run.")
@click.option("--logs-days", type=int, default=30, show_default=True)
@click.option("--audit-days", type=int, default=365, show_default=True)
@click.option("--actions-days", type=int, default=365, show_default=True)
@click.option("--comments-days", type=int, default=365, show_default=True)
@click.option("--sessions-days", type=int, default=180, show_default=True)
@click.option("--population-days", type=int, default=90, show_default=True)
...
def privacy_purge(
    ...
    with enter_session() as sess:
        results = purge_old_personal_data(
            sess,
            dry_run=not execute,
            logs_days=logs_days,
            audit_days=audit_days,
            actions_days=actions_days,
    ...
    if not execute:
        click.echo("Dry-run only. Re-run with --execute after reviewing the result.")
